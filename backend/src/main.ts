...
import { initObservability, shutdownTracing } from './infra/observability';
...
async function bootstrap(): Promise<void> {
  ...
  // Initialises Sentry from validated configuration and reports whether tracing
  // started — see `infra/observability`. Called before the port is bound so the
  // boot log states once, unambiguously, what is actually reporting.
  ...
  // Flush buffered spans before the process exits: the requests most likely to
  // explain an incident are the last ones before a crash or a deploy, and an
  // unflushed span is a span that never arrives. `SIGTERM` is what a rolling
  // deploy sends; `SIGINT` covers Ctrl-C in development.
  ...
  if (!config.isProduction) {
    // The document definition lives in `config/openapi.ts` so the same document
    // is served here and generated by `openapi:generate` in CI.
    SwaggerModule.setup(`${config.apiPrefix}/docs`, app, buildOpenApiDocument(app), {
